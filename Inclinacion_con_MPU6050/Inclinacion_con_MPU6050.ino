...
void prender_D(int x){
  for (n = 0; n < x+1; n++){
    digitalWrite(Leds2[n],HIGH);
  }
  for (n = x+1; n < 3; n++){
    digitalWrite(Leds2[n],LOW);
  }
}

void prender_I(int x){
  for (m = 0; m < x+1; m++){
...
Serial.begin(115200);
  ...
  for(int pin = 0; pin < 3; pin++){
    ...
    pinMode(Leds2[pin], OUTPUT);
    digitalWrite(Leds[pin],LOW);
    digitalWrite(Leds2[pin],LOW);
  }
